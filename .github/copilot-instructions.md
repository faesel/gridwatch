...
| `~/.copilot/logs/process-<ms-timestamp>-<pid>.log` | Token utilisation lines, compaction start/complete events, persisted checkpoint summaries, and `Registering/Unregistering foreground session: <uuid>` lines used to attribute token data to sessions |
...
interface SessionData {
  ...
  contextWindow?: number          // denominator from the peak-tokens log line (varies by model; NOT always 128K)
  initialTokens?: number          // first recorded token count for the session
  initialUtilisation?: number     // first recorded utilisation %
  tokenLogPruned: boolean         // true when Copilot pruned this session's process log AND no snapshot exists
  tokenStatsCached?: boolean      // true when token stats came from the gridwatch.json snapshot, not a live log
...
Copilot's context window varies by model (observed 168K / 200K / 272K / 922K). Always parse the denominator from the `Utilization X% (used/total tokens)` line — never hardcode 128000. `peakTokens`, `peakUtilisation` and `contextWindow` must all be taken from the **same** (max-tokens) log line so they stay coherent.

### Token log pruning + persisted snapshots
Copilot prunes `~/.copilot/logs` to ~14 days, so older sessions lose their live token data. GridWatch persists a `tokenStats` snapshot into each session's `gridwatch.json` for the first page (top `TOKEN_SNAPSHOT_LIMIT = 20` most-recently-updated sessions) on load. Resolution is 3-tier: live log → persisted snapshot (`tokenStatsCached`) → none. `tokenLogPruned` is only true when there is no live data, no snapshot, logs exist, and the session predates the oldest surviving log. Snapshot writes are change-gated, atomic (temp+rename) and serialised per session via `mutateSessionMeta` to avoid clobbering tags/notes.
